...
make_rj_sbox(){
    ...
    (( y=((y<<1)&0xff)|(y>>7), sb^=y, y=((y<<1)&0xff)|(y>>7), sb^=y ));
    #(( sbox[x]!=(sb^0x63) )) && { printf "$FUNCNAME: sbox[%02x]=%02x != sbox(%02x)=%02x\n" $x ${sbox[x]} $x $((sb^0x63)); exit 1; }
    (( sbox[x]=sb^0x63 ));
}
...
make_rj_sbox_inv(){
    ...
    #    printf "$FUNCNAME: sbox_inv[$x]=%02x != sbox_inv($x)=%02x\n" ${sbox_inv[x]} $((gf_mulinv[sb])); exit 1; }
    (( sbox_inv[x]=gf_mulinv[sb] ));
}
